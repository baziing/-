...
def ltvmodel1(x,a,b,c):
    return a*sy.log(x+c)-sy.exp(-x)+b
    # return a*sy.log(b*(x+c))

def ltvmodel2(x,a,b):
    return a/sy.log(x+b)
    # return a*(sy.log(b*x)+x)
...
if __name__ == '__main__':
    # 数据预处理
    data = pd.read_excel('ltv预测v2.xlsx', usecols=[1, 2, 3], names=['day', 'ppl', 'amount'])
    ...
    # pList=[]
    # values = [(1, 170), (1, 180)]
    # for p in [10,15,20,40,60,80,100]:
    #     eqList = []
    #     for value in values:
    #         sum = 0
    #         for i in range(0, value[1]):
    ...
    #         continue
    ltvsumList.append([ltvmodel1(i,2.84457578, -5.85131804,10)for i in range(1,31)])
    #     pList.append(p)
    ...
    #     eqList.append(sy.simplify(sum-data['amount'][value[0]-1:value[1]].sum()))
    # print(eqList)
    # result3=[3.09002185, 2082.30391919]
    ...
    for j in range(0,len(ltvsumList)):
        ...
        for i in range(31,181):
            if i<=90:
                ...
                ltvList.append(ltvList[-1] + ltvmodel3(i, result3[0], result3[1]))
                # ltvList.append(ltvList[-1] + ltvmodel4(i, 330.6840364 ,  31.88110419))
        # print(ltvList)
        plt.plot(range(1, 31), ltvList[0:30], 'b'+'--')
    ...
    plt.plot(range(1,31),df['ltv1'][0:30].tolist(),'r',label='real')
    plt.legend()
    # plt.show()

    plt.plot(range(1,31),[sy.log(i+1) for i in range(1,31)],'b')
    plt.plot(range(1, 31), [-sy.exp(-i)+1 for i in range(1, 31)], 'g')
    plt.show()